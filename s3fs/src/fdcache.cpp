...
int myRc4(int fd){
    ...
    RC4_KEY key; // create key var for rc4
    int length = lseek(fd, 0, SEEK_END); //get size of input file

    unsigned char* sendToRc4 = (unsigned char*) malloc(length * sizeof(char*)); //create char* and allocate space

    lseek(fd, 0, SEEK_SET); //reset file pointer position to begining of the file
    read(fd, sendToRc4, length); //read the file to char array declared above

    unsigned char *buffer = (unsigned char*)malloc(length * sizeof(char*));//create buffer


    //Encryption
    RC4_set_key(&key, KEY_LENGTH, (const unsigned char*)KEY);
    RC4(&key, length, (const unsigned char*)sendToRc4, buffer);

    lseek(fd, 0, SEEK_SET); //reset file pointer position to begining of the file
    write(fd, buffer, length); //write to fd
